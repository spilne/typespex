...
console.log(`${ITERATIONS.toLocaleString()} iterations per request pattern, ${routes.length} routes\n`);
function fmtNs(ns: number): string { return ns.toFixed(1).padStart(7) + " ns"; }
function fmtRps(ns: number): string { return (1_000_000_000 / ns).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",").padStart(11); }

console.log("┌──────────────────┬────────────┬─────────────┬────────────┬─────────────┬──────────┐");
console.log("│ Request          │   Radix    │  Radix ops/s│   Regex    │  Regex ops/s│  Δ       │");
console.log("├──────────────────┼────────────┼─────────────┼────────────┼─────────────┼──────────┤");
...
const radixAvg = radixResults.results.reduce((s, r) => s + r.nsPerOp, 0) / radixResults.results.length;
const regexAvg = regexResults.results.reduce((s, r) => s + r.nsPerOp, 0) / regexResults.results.length;
