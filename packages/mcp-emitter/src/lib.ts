...
export interface McpEmitterOptions {
  /** A unique, non-empty selection of generation modes. Defaults to native. */
  readonly mode?: readonly McpMode[];
  readonly "application-module"?: string;
  /** Library-only generation is the default. */
...
export const $lib = createTypeSpecLibrary({
  ...
  diagnostics: {
    ...
    "missing-application-module": {
      severity: "error",
      messages: {
        default: "application-module is required when MCP launchers are enabled.",
      },
    },
    ...
    "bridge-http-library-missing": {
      severity: "error",
      messages: { default: "HTTP bridge generation requires @typespec/http 1.14 or newer." },
    },
    "bridge-unsupported": {
      severity: "error",
      messages: {
