...
function emitSchemaDefinition(
  ...
  return `const ${name} = createTypeSpecSchema${typeArguments}({
...
import { mcpHttpBridgeOperations } from "./${server.fileNames.httpClient}.js";`
  ...
  const runtimeApplication = server.modes.httpBridge
...
`;
  ...
  switch (launcher) {
    case "stdio":
      return `${common}import { serveTypespexStdio } from "@typespex/mcp-transport-stdio";

export const ${camelCase(server.symbolName)}McpStdioServer = serveTypespexStdio(
  ${factory},
  application.stdio,
);
`;
...
const mcpHandler = createTypespexHttpHandler(${factory}, application.http);
