...
bindings backed by `@typespex/http-server`.

## Entry points

- `@typespex/http-emitter` exports the TypeSpec emitter and its library definition.

Configure the package as an emitter in `tspconfig.yaml`. Generated files import
`@typespex/http-server`, which is an optional aligned peer of the emitter and must be installed by
...
required. Compatible releases from 1.14.0 through the latest 1.x versions of
