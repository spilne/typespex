...
describe("isContentTypeAccepted", () => {
  ...
  test("matches exact media type ignoring case and parameters", () => {
    ...
    expect(isContentTypeAccepted("Application/JSON; charset=utf-8", ["application/json"])).toBe(true);
    expect(isContentTypeAccepted("multipart/form-data; boundary=abc", ["multipart/form-data"])).toBe(true);
  });
  ...
  test("matches when any declared entry matches", () => {
    ...
    expect(isContentTypeAccepted("application/xml", ["application/json", "text/plain"])).toBe(false);
  });
});
